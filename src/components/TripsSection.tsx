        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {trips.map((item, index) => (
            <ScrollReveal key={item.id} delay={index * 0.1}>
              <div className="glass rounded-2xl p-6 group hover:border-earth/30 transition-all duration-500 h-full">
                <div className="flex items-start gap-4 mb-4">
                  <span className="text-2xl flex-shrink-0">{getFlagEmoji(item.location)}</span>
